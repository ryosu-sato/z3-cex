...
let rec print_list_aux_with_flag ?(first = true) print punc fm xs =
  ...
  | x :: xs ->
      ...
      Format.fprintf fm "%a"
        (print_list_aux_with_flag print punc ~first:false)
        xs
...
let print_list_with_flag print ?open_box ?(first = false) ?(last = false) ?limit
    punc fm xs =
  if xs <> [] then (
    let exceed =
      match limit with
      | None -> false
      | Some limit -> limit >= 0 && List.length xs > limit
    in
    let xs = if exceed then BatList.take (Option.get limit) xs else xs in
    let punc' = format_of_string punc in
    (match open_box with
    | None -> Format.fprintf fm "@["
    | Some open_ -> open_ fm);
...
let print_list print ?open_box ?(first = false) ?(last = false) ?limit punc fm
    xs =
  let print ~first ~last fm x = print fm x in
...
module Pair = struct
  ...
  let of_list xs =
    match xs with [ x; y ] -> (x, y) | _ -> invalid_arg "%s" __FUNCTION__
...
module Triple = struct
  ...
  let print f g h ppf (x, y, w) =
    Format.fprintf ppf "@[(@[%a,@ %a,@ %a@])@]" f x g y h w
end
...
module Option = struct
  ...
  let ( let** ) x f =
    match x with None -> None | Some y -> ( try f y with _ -> None)
...
module List = struct
  ...
  let rec init n f i acc_rev =
    if i >= n then rev acc_rev else init n f (i + 1) (f i :: acc_rev)
  ...
  let filteri f xs =
    filter_map Fun.id @@ mapi (fun i x -> if f i x then Some x else None) xs

  let rev_split xs =
    fold_left (fun (acc1, acc2) (x, y) -> (x :: acc1, y :: acc2)) ([], []) xs
  ...
  let rec find_mapi f i = function
    | [] -> raise Not_found
    | x :: xs' -> (
        match f i x with None -> find_mapi f (i + 1) xs' | Some r -> r)
  ...
  let rec find_map_opt f xs =
    ...
    | [] -> None
    | x :: xs' -> (
        match f x with None -> find_map_opt f xs' | Some y -> Some y)

  let find_map f xs =
    match find_map_opt f xs with None -> raise Not_found | Some x -> x

  let find_map_default f x xs =
    match find_map_opt f xs with None -> x | Some x -> x
  ...
  let find_decomp_opt f xs =
    try Some (find_decomp f xs) with Not_found -> None

  (* For the case when the optional argument is an obstacle of the type infe9rence *)
  let assoc_ x xs = snd @@ find (( = ) x -| fst) xs

  let assoc_default ?(eq = ( = )) x k tbl =
    try assoc ~eq k tbl with Not_found -> x

  let assoc_default_id ?(eq = ( = )) k tbl =
    try assoc ~eq k tbl with Not_found -> k

  let assoc_opt ?(eq = ( = )) k tbl =
    try Some (assoc ~eq k tbl) with Not_found -> None
  ...
  let remove_lower is_lower xs =
    let rec aux acc_rev xs =
      ...
      | x :: xs' ->
          let acc_rev' =
            if exists (is_lower x) acc_rev || exists (is_lower x) xs' then
              acc_rev
            else x :: acc_rev
          in
          aux acc_rev' xs'
...
module Exception = struct
  ...
  let default v f x = try f x with _ -> v
end
  ...
  let rec loop edges roots xs rev_acc =
    ...
    | r :: roots' ->
        ...
        let roots'' =
          ...
          List.filter_out (fun y -> List.exists (snd |- eq y) edges2) ys
          @ roots'
        in
...
module Compare = struct
  ...
  let topological ?(eq = ( = )) ?dom edges =
    let map =
      ...
      let no_edge =
        List.filter_out
          (fun x -> List.exists (fun (y, z) -> eq x y || eq x z) edges)
          dom'
      in
      edges |> topological_sort ~eq |> ( @ ) no_edge
...
module String = struct
  ...
  let sign_to_letters s =
    let map c =
      match of_sign c with None -> make 1 c | Some s -> "_" ^ s ^ "_"
    in
    replace_chars map s
  ...
  let split_blanc s =
    ...
    let rec parse quot acc cs =
      ...
      | None, _, c :: cs' when Char.is_whitespace c ->
          of_list acc :: parse None [] cs'
...
module Math = struct
  let rec gcd m n =
    if m < n then gcd n m else if n = 0 then m else gcd n (m mod n)
...
module Random = struct
  ...
  let string
      ?(cs = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz") n
      =
...
module Arg = struct
  ...
  let align args =
    args
    |> List.map (fun (s, f, desc) ->
           if s = "" then (s, f, " " ^ desc) else (s, f, desc))
    |> align
    |> List.mapi (fun i (s, f, desc) ->
           if s = "" then
             let nr = if i <> 0 then "\n  " else "" in
             let s' =
               desc |> String.trim |> Format.sprintf "%s(*** %s ***)" nr
             in
             (s', f, " ")
           else
             let aux c =
               if c = '\n' then
                 let rec len i =
                   if desc.[i] = ' ' then 1 + len (i + 1) else 3
                 in
...
module Sys = struct
  ...
  let runtime_parameters_list () =
    !!runtime_parameters |> String.split_on_char ','
...
module Marshal = struct
  ...
  let from_file file default =
    if Sys.file_exists file then IO.CPS.open_in file Marshal.from_channel
    else default ()
...
module Lexing = struct
  ...
  let print_position fm { pos_fname; pos_lnum; pos_cnum; pos_bol } =
    Format.fprintf fm {|File "%s", line %d, column %d|} pos_fname pos_lnum
      (pos_cnum - pos_bol)
...
module Time = struct
  ...
  let string_of_tm
    ...
    Format.sprintf "%04d/%02d/%02d %02d:%02d:%02d" (tm_year + 1900) (tm_mon + 1)
...
let transitive_closure ?(eq = ( = )) edges =
  ...
  let cons (x, y) es =
    if List.mem ~eq:eq' (x, y) es then es else (x, y) :: es
  in
  let f edges' =
    let aux acc (x, y) =
      List.fold_left
        (fun acc' (z, w) -> if eq y z then cons (x, w) acc' else acc')
        acc acc
    in
...
let assign_id_by_eq (type t) ?(eq = ( = )) ?(size = 0) ?(origin = 0)
    (xs : t list) : int * (t -> int) * (t * int) list =
...
let topological_index (scc_of : (int * int) list -> int list list) ?(eq = ( = ))
  ...
  let _, id_of, map =
    assign_id_by_eq ~eq @@ List.rev_flatten_map Pair.to_list edges
  in
  ...
  let repr_of x =
    List.find_map (fun (r, xs) -> if List.mem x xs then Some r else None) repr
  in
  ...
  repr
  |> List.rev_flatten_map (fun (r, xs) ->
         List.map (Pair.pair @@ List.assoc r idx_repr) xs)
